...
var addAttributes = function addAttributes(element, attrs) {
  ...
  props.forEach(function (_ref) {
    ...
    if (utils.isCustomElement(element)) return element.props = _objectSpread({}, element.props, _defineProperty({}, propName, propValue));
    ...
    if (propName.toLowerCase() in element && typeof propValue === 'function') {
      var eventName = utils.getEventName(propName);
      return element.addEventListener(eventName, propValue);
    }

    if (propName === 'xlinkHref') return element.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', propValue);
    element.setAttribute(propName, propValue);
  });
  return element;
};

var fillFragmentByChildren = function fillFragmentByChildren(fragment, child) {
  if (child) {
    if (typeof child === 'string' || typeof child === 'number') {
      var textnode = document.createTextNode(child);
      fragment.appendChild(textnode);
    } else if (child instanceof Array) {
      child.forEach(fillFragmentByChildren.bind(_this, fragment));
    } else {
...
var createFragmentWithChildren = function createFragmentWithChildren(children) {
  ...
  children.forEach(fillFragmentByChildren.bind(_this, fragment));
  return fragment;
};

var jsx = function jsx(tagName, attrs) {
  // create main element
  var element = utils.isSVG(tagName) ? document.createElementNS('http://www.w3.org/2000/svg', tagName) : document.createElement(tagName); // if we have custom element we should add all attributes to it props
  // but first of all we should create props object of this element

  if (utils.isCustomElement(element)) element.props = {}; // create children and append it to the main element
  ...
  element.appendChild(fragment); // add attributes to the main element.
